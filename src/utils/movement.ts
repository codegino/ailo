...
export function move(
  ...
  if (direction === MOVE_DIRECTION.RIGHT) {
    return {
      ...coordinate,
      x: coordinate.x === dimension.width - 1 ? 0 : ++coordinate.x,
    };
  } else if (direction === MOVE_DIRECTION.LEFT) {
    return {
      ...coordinate,
      x: coordinate.x === 0 ? dimension.width - 1 : --coordinate.x,
    };
  } else if (direction === MOVE_DIRECTION.UP) {
    return {
      ...coordinate,
      y: coordinate.y === 0 ? dimension.height - 1 : --coordinate.y,
    };
  } else if (direction === MOVE_DIRECTION.DOWN) {
    return {
      ...coordinate,
      y: coordinate.y === dimension.height - 1 ? 0 : ++coordinate.y,
    };
  } else {
